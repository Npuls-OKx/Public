...
**Aanleiding.** De centrale onderwijsspecificatie-payload draagt een `regelsets`-array en een `regelsetVerwijzingen`-veld, maar zegt er expliciet bij dat de **interne structuur van een regelset** er niet in staat. Daarmee was afgesproken dát kiesbaarheid en voorwaarden buiten de specificatie leven, maar niet hoe zo'n regel eruitziet. Dit document geeft die vorm.

**Context.** Eerste concretisering van de [eisen rond keuzes bij onderwijsspecificaties](../../../Referentiemateriaal/requirements/keuzes-rond-onderwijsspecificaties.md) (R1 tot en met R16). Scenario is leerroute 1 aan de hand van persona [Jochem](../../../Referentiemateriaal/persona's/jochem.md), opleiding Apothekersassistent. De waarden in het voorbeeld zijn indicatief: ze illustreren de vorm en de samenhang, niet de inhoud van een bestaande opleiding.
...
- [LR1](../../../Referentiemateriaal/persona's/jochem.md "persona Jochem, leerroute 1"): de keuzedeelruimte van Apothekersassistent als voorbeeld. De vorm is generiek voor alle keuzes rond onderwijsspecificaties, op elk niveau (R16).
- Buiten scope: de evaluatie-implementatie (wie rekent wanneer), de studentgegevens die daarvoor nodig zijn (raakt ADR 0009), Vrijere keuzevormen zijn uitdrukbaar (§5.1) en deels al in het voorbeeld gebruikt (maximaal 2, precies 1); wat [LR1-3](https://github.com/Npuls-OKx/meta/blob/d47bb0c74ec899a4384d06331692f74b9bd1db58/architecture/docs/specificatie/okx-oeapi-consumer-profiel/doc/20260501_Specificatie_document_OKx_OEAPI_profiel.md "leerroutes en persona's in het OEAPI-profiel") werkelijk vraagt bepaalt de vaststelling.

## 4. Ontwerpkeuzes

- **Generiek over alle onderwijsspecificaties (R16).** Een regelset kadert af wat een student kan kiezen en is toepasbaar op **elke** onderwijsspecificatie als keuzecontext. "Keuzedeel" is de mbo-specifieke invulling binnen [LR1-3](https://github.com/Npuls-OKx/meta/blob/d47bb0c74ec899a4384d06331692f74b9bd1db58/architecture/docs/specificatie/okx-oeapi-consumer-profiel/doc/20260501_Specificatie_document_OKx_OEAPI_profiel.md "leerroutes en persona's in het OEAPI-profiel"); de regelvorm kent dat onderscheid niet.
...
[LR1](../../../Referentiemateriaal/persona's/jochem.md "persona Jochem, leerroute 1"), indicatief. De specificatie- en leeruitkomst-id's zijn illustratief; na samenvoeging met de koppelingen-lijn (#119) verwijzen ze naar de id's uit de centrale onderwijsspecificatie-payload.
...
- **JSON Schema en gegenereerde bomen ontbreken nog.** De andere payload-documenten in deze map dragen een JSON Schema (draft 2020-12) plus een schemaboom en instantiebomen, gegenereerd met `scripts/json-tree.py`; dit document heeft alleen het voorbeeld. Een schema legt vast welke velden verplicht zijn en welke waarden een veld mag dragen, en is daarmee een inhoudelijke vaststelling die aansluitend gemaakt moet worden. Zolang het ontbreekt is de vorm van een regelset niet machinaal te valideren ([U8](../../uitgangspunten.md#u8-machine-interpreteerbaar-met-leesbare-weergaven)).
- De id's in dit document verwijzen nog niet naar de [centrale onderwijsspecificatie-payload](payload-onderwijsspecificatie.md); die koppeling volgt bij het samenvoegen van beide payloads.

## 10. Gerelateerde uitwerkingen

- [Requirements-voorstel keuzes rond onderwijsspecificaties](../../../Referentiemateriaal/requirements/keuzes-rond-onderwijsspecificaties.md) (R1-R16, de bron van dit document).
